...
from transformers.models.llama.modeling_llama import LlamaAttention, LlamaFlashAttention2, LlamaSdpaAttention, apply_rotary_pos_emb, repeat_kv

def compute_attention_with_qkv(self, query_states, key_states, value_states, attention_mask, causal_mask):
...
def setup_online_kv_for_llama(model):
    for each in model.model.layers:
        if type(each.self_attn) is LlamaAttention:
            each.self_attn.forward = online_kv_forward.__get__(each.self_attn, LlamaAttention)
        elif type(each.self_attn) is LlamaFlashAttention2:
            each.self_attn.forward = online_kv_flash_forward.__get__(each.self_attn, LlamaFlashAttention2)
        elif type(each.self_attn) is LlamaSdpaAttention:
            each.self_attn.forward = online_kv_sdpa_forward.__get__(each.self_attn, LlamaSdpaAttention)
        else:
            print(type(each.self_attn))
            raise NotImplementedError
    return model

# Load the model and tokenizer
model_name = "meta-llama/Meta-Llama-3-8B-Instruct"
model = AutoModelForCausalLM.from_pretrained(model_name, device_map = "auto", token = "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")
tokenizer = AutoTokenizer.from_pretrained(model_name)

# Define a prompt and custom KV cache (if any)
messages = [
...
model = setup_online_kv_for_llama(model)
outputs = model.generate(
    input_ids,
    max_new_tokens=256,
    eos_token_id=terminators,
